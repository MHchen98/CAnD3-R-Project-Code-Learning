...
ggplot(data = gapminder |> 
       ...
       mapping = aes(x = lifeExp,
                     ...
                     colour = continent)) +
  geom_freqpoly(mapping = aes(y = after_stat(density)), # *width, to get the percentage
                binwidth = 5)
...
ggplot(gapminder |>  
  ...
  theme(plot.title = element_text(face = "bold"),
        ...
        legend.key.size = unit(1, "cm"),
        legend.box = "vertical",
        legend.title = element_text(hjust = 0.5)) + # Centring legend title.
  # guides(size = guide_bins(# Push legend title to the bottom:
  #   title.position = "bottom")) + ## guide_bins() no longer accepts title.position or title.hjust after 3.4.0
...
aus.fert |> 
  as_tibble() |> 
  filter(!Group == "total", !Age > 100) |> 
  ggplot(aes(x = Year, y = Age, fill = ntile(Fertility, 100))) +
  facet_wrap(~paste0(str_to_title(Group),"s"), nrow = 2) +
  geom_raster() +
  # scale_fill_viridis_c(option = "magma", direction = -1) +
  scale_fill_continuous_sequential(palette = "SunsetDark", 
                                   labels = function(x) paste0(round(x/100, 1), "%")) +
  scale_x_continuous(breaks = seq(min(aus.fert$Year), max(aus.fert$Year), by = 20)) +
  labs(title = "Fertility in Australia",
       subtitle = "1921 - 2002",
       x = "", 
       fill = "Fertility Rate (Percentile)") +
  guides(fill = guide_legend(nrow = 1, 
                             title.position = "top", 
                             label.position = "bottom")) +
  theme(plot.title = element_text(size = 25),
...
aus.fert |> 
  ...
  ggplot(aes(x = Year, y = Age, fill = ntile(Fertility, 100))) +
  facet_wrap(~paste0(str_to_title(Group), "s"), nrow = 2) +
  geom_tile() +   # swapped geom_raster() for geom_tile()
  scale_fill_continuous_sequential(palette = "SunsetDark") +
  scale_x_continuous(breaks = seq(min(aus.fert$Year), max(aus.fert$Year), by = 20)) +
  labs(title = "Fertility in Australia",
       subtitle = "1921 - 2002",
...
mat <- aus.fert %>%
  filter(!Group == "total", !Age > 100) %>%
  mutate(Fertility = Fertility / 100) |>   # convert to proportion
  select(Year, Age, Fertility) %>%
  pivot_wider(names_from = Year, values_from = Fertility) %>%
  arrange(Age)

Fertility <- as.matrix(mat[,-1])        # Fertility matrix
Years <- as.numeric(colnames(z))    # Years
Age <- mat$Age                    # Ages

# Generate a SunsetDark palette (100 colors)
cols <- sequential_hcl(100, palette = "SunsetDark")

# Plotly 3D surface
plot_ly(x = Years, y = Age, z = Fertility) %>%
  add_surface(colorscale = cols) %>%
  layout(
    title = "3D Fertility Surface (Australia)",
    scene = list(
...
# Create Lexis-style heatmap
ggplot(aus_grid, aes(x = Year, y = Age, fill = Fertility)) +
  geom_tile(color = NA) +  # heatmap for fertility
  scale_fill_continuous_sequential(palette = "SunsetDark", name = "Fertility Rate") +
  
  # Highlight a cohort: 1950 (Year - Age = 1950)
  geom_abline(intercept = -1950, slope = 1, color = "black", linewidth = 1) +
  
  # Highlight age 25
  geom_hline(yintercept = 25, color = "lightseagreen", linewidth = 1) +
  
  # Highlight year 1980
  geom_vline(xintercept = 1980, color = "pink", linewidth = 1) +
  
  # Annotation
  annotate("text", x = 1970, y = 33.5, label = "1950 Cohort",
